...
def login():
    ...
    try:
        ...
        if os.environ.get('ENVIRONMENT') == 'production':
            ...
            host = os.environ.get('APP_URL', 'allervie-test-deployment-xjfjs.ondigitalocean.app')
...
def callback():
    ...
    try:
        ...
        # Create a new flow from the client config, determine the correct redirect URI
        if os.environ.get('ENVIRONMENT') == 'production':
            # For Digital Ocean App Platform deployment
            host = os.environ.get('APP_URL', 'allervie-test-deployment-xjfjs.ondigitalocean.app')
            correct_redirect = f"https://{host}/api/auth/callback"
